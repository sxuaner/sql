-- This file is located at /Users/xuansong/projects/databases/oracle/2020-3-7.sql
...
-- 
select expiry_date from dba_users where username = 'TMP';

-- Following line of code is not working because dba.users is a virtual table
-- what is a virtual table?
update dba_users set expiry_date= '2032-12-12' where username = 'SYS';

-- This does the trick
ALTER PROFILE DEFAULT LIMIT PASSWORD_LIFE_TIME <number of life time>/UNLIMITED;

-- To check a users profile
SELECT PROFILE FROM dba_users WHERE USERNAME='HR';

-- list out all usernames and their expriy_date
select username, expiry_date from dba_users;

-- show all info in dba_users table for debugging
SELECT * FROM dba_users ;

-- To change the parrword life time of default profile to unlimited (only for open users)
ALTER PROFILE default LIMIT PASSWORD_LIFE_TIME unlimited ;

-- unlock user HR
ALTER USER HR IDENTIFIED BY hr ACCOUNT UNLOCK;

-- now check dba_users again , the password life time of HR is null, which means unlimited.
...
-- How to lift the limitation of 1000 rows max in table?
